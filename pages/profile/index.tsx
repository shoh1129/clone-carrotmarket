...
import { cls, getImageURL } from "@lib/client/utils";
...
const Profile: NextPage = () => {
  const { user } = useUser();
  const { data } = useSWR<ReviewsReponse>('/api/reviews')
  return (
    <Layout hasTabBar title="나의 캐럿">
      <div className="px-4">
        <div className="flex items-center mt-4 space-x-3">
          {
            user?.avatar ? (
              <img
                src={getImageURL(user?.avatar, 'public')}
        ...
        {
          data?.reviews?.map((review: ReviewWithUser, i: number) => {
            return (
              <>
                <div className="mt-12">
